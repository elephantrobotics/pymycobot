...
from pymycobot.myagvapi import CommunicationProtocol, Utils, setup_serial_connect
...
class MercuryL1ChassisDataException(Exception):
    pass

class MercuryL1ChassisCommand(CommunicationProtocol):
    genre_timeout_table = {
        ...
        for _ in range(5):
            if ProtocolCode.SET_COMMUNICATION_MODE.equal(genre) and self._communication_mode == 2:
                ...
                if is_save_mac_addr is False:
                    ...
                    info = f"Mercury L1 Chassis Bluetooth MAC Address: {mac_addr}"
                    self._save_file("L1_CHASSIS_BLUETOOTH_MAC_ADDR", info)
    ...
    def _instruction_decoding(cls, genre, reply_data):
        ...
        if genre in (
                ...
                ProtocolCode.GET_LIFT_CONTROL_MSG,
        ):
            respond = []
...
class MercuryL1Chassis(MercuryL1ChassisCommand):
    ...
    def __init__(self, port, baudrate=1000000, timeout=0.1, debug=False, save_serial_log=False):
        ...
        self._serial_filename = 'L1_chassis_serial_data.log'
    ...
    def move_forward(self, speed):
        ...
        try:
            ...
            raise MercuryL1ChassisDataException(f"Invalid speed: {speed}")

        speed = round(speed, 2)
        if not 0.01 <= speed <= 1.50:
            raise MercuryL1ChassisDataException("Speed must be between 0.01 and 1.50.")
    ...
    def set_auto_report_state(self, state):
        ...
        if state not in (0, 1):
            raise MercuryL1ChassisDataException("State must be 0 or 1")
    ...
    def set_motor_enable(self, motor_id, state):
        ...
        Returns:
            int: 1: Success, 0: Failed
        """
        if motor_id not in (1, 2, 254):
            raise MercuryL1ChassisDataException("Motor id must be in (1, 2, 254)")
    ...
    def set_communication_state(self, state):
        ...
        if state not in (0, 1, 2):
            raise MercuryL1ChassisDataException("State must be 0, 1 or 2")
    ...
    def set_led_color(self, position, color, brightness=255):
        ...
        if position not in (0, 1):
            raise MercuryL1ChassisDataException("Position must be 0 or 1")

        if not 0 <= brightness <= 255:
            raise MercuryL1ChassisDataException("Brightness must be between 0 and 255")

        if len(color) != 3:
            raise MercuryL1ChassisDataException("Color must be a tuple of 3 values")

        if any(map(lambda c: not 0 <= c <= 255, color)):
            raise MercuryL1ChassisDataException("Color must be between 0 and 255")

        if not isinstance(color, tuple):
            raise MercuryL1ChassisDataException("color must be a tuple")
    ...
    def set_led_mode(self, mode):
        ...
        """
        if mode not in (0, 1):
            raise MercuryL1ChassisDataException("Mode must be 0 or 1")
    ...
    def set_pin_output(self, pin, state):
        ...
        """
        if not 1 <= pin <= 6:
            raise MercuryL1ChassisDataException("Pin must be between 1 and 6")
    ...
    def set_lift_control(self, mileage, speed):
        """Lifting Motion Control
        Args:
            mileage(int): Lifting mileage, range is 0 ~ 450 mm
            speed(int): Lifting speed, range is 0 ~ 100

        Returns:
            int: 1: Success, 0: Failed
        """
        if not -450 <= mileage <= 450:
            raise MercuryL1ChassisDataException("Mileage must be between -450 mm and 450 mm")
        if not 1 <= speed <=100:
            raise MercuryL1ChassisDataException("Speed must be 1 or 100")
        return self._merge(ProtocolCode.SET_LIFT_CONTROL_MOVE, [mileage], speed)

    def get_lift_information(self):
        """Read Lift Information
