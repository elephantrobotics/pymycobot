...
import numpy as np
import serial
from pymycobot.e1_close_loop import E1CloseLoop
from pymycobot.robot_info import _interpret_status_code, RobotStatusE1Info
from pymycobot.common import ProtocolCode


class MercuryE1(E1CloseLoop):
    def __init__(self, port="COM3", baud_rate="1000000", timeout=0.1, debug=False, save_serial_log=False):
        """
        Args:
            port     : port string
            baud_rate : baud rate string, default '1000000'
            timeout  : default 0.1
            debug    : whether show debug info
        """
        super(MercuryE1, self).__init__(debug)
    ...
    def _mesg(self, genre, *args, **kwargs):
        read_data = super(MercuryE1, self)._mesg(genre, *args, **kwargs)
        ...
        #         res.append(byte_value)
            ...
            res_list = [i for i in valid_data]
            return res_list
        elif data_len == 21 and genre == ProtocolCode.TOOL_SERIAL_WRITE_DATA:
            res_list = [i for i in valid_data]
            return res_list
        else:
            if genre in [
        ...
        if genre in [
            ...
            if len(res) == 37:
                ...
                info = RobotStatusE1Info.error_info[self.language]
    ...
    def go_home(self, speed=20, _async=False):
        ...
        Return:
            ...
            0 : failed.
        """
        return self.send_angles([0, 0, 0, 0, 0, 0, 0], speed, _async=_async)
    ...
    def set_mit_control(self, joint_id, joint_pos, rad_speed, kp, kd, torque):
        ...
        self.calibration_parameters(
            class_name=self.__class__.__name__, joint_id=joint_id,
            mit_q=joint_pos, mit_dq=rad_speed, mit_kp=kp, mit_kd=kd, mit_tau=torque)
        data = self._encode_mit_control(joint_pos, rad_speed, kp, kd, torque)
        return self._mesg(ProtocolCode.SET_MIT_CONTROL, joint_id, *data)

    def set_mit_controls(self, controls):
        """MIT all-joint control.

        Args:
            controls (list): 7 MIT control groups. Each group can be
        ...
        """
        self.calibration_parameters(class_name=self.__class__.__name__, mit_controls=controls)
        ...
        return self._mesg(ProtocolCode.SET_MIT_CONTROLS, *data)
