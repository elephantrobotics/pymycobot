...
- **功能：** 向机械臂发送一个关节度数
- **参数：**
  - `id`：关节 id（`genre.Angle`），范围 int 1-4
  - `degree`：度数值（`float`）
      | 关节 Id | 范围 |
      | ---- | ---- |
      | 1 | -162 ~ 162 |
...
- **功能：** 点动控制角度
- **参数**：
  - `joint_id`：表示机械臂的关节，用关节ID表示，范围是1~4
...
- **返回值**：`int` 机器人当前所设定的运行速度，范围在1-100
...
服务端文件在`demo文件夹`中，具体请检查demo文件夹中的[Server_260.py](../demo/Server_260.py)文件

### socket 控制

> 注意：
> 大部分方法与 MyPalletizer260 类相同，这里只列出新方法。
