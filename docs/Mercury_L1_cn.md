...
- **参数**：
  - `arm_id`: (`int`) 手臂ID
    - `1`: 左臂
    - `2`: 右臂
  - `hand_id`: (`int`) 五指设备ID，默认2，取值范围 2 ~ 254。
...
- **参数**： 
  ...
  - `rank_mode` (`int`) 0 ~ 5  ，合拢程度，等级越高越合拢。
...
- **参数**：
  ...
  - `arm_id` `int`：机械臂类型
    - `0`：左右臂同时控制（使用同一组 MIT 参数）
    - `1`：左臂
    - `2`：右臂

  - `joint_id` `int`：关节 ID，范围 `1 ~ 7`

  - `joint_pos` `float`：目标关节位置，单位 `rad`
  ...
  - `rad_speed` `float`：目标关节速度，单位 `rad/s`
    - J1 ~ J6：范围 `-20 ~ 20`
    - J7：范围 `-30 ~ 30`

  - `kp` `float`：位置增益，范围 `0 ~ 500`

  - `kd` `float`：速度增益，范围 `0 ~ 5`

  - `torque` `float`：前馈力矩，单位 `Nm`
    - J1 ~ J3：范围 `-120 ~ 120`
    - J4 ~ J6：范围 `-28 ~ 28`
    - J7：范围 `-10 ~ 10`
...
- **参数**：
  ...
  - `left_controls` `list`：左臂 MIT 控制参数列表，仅在 `arm_id=0` 或 `arm_id=1` 时使用

  - `right_controls` `list`：右臂 MIT 控制参数列表，仅在 `arm_id=0` 或 `arm_id=2` 时使用
...
- **说明**：
  ...
  - 每个控制组对应一个关节
  - 每个机械臂需提供 7 组控制参数，对应 J1 ~ J7
  - `joint_pos`、`rad_speed`、`kp`、`kd`、`torque` 的取值范围与 `set_mit_control()` 相同
  - 当 `arm_id=1` 时，仅发送左臂参数，右臂自动补零
  - 当 `arm_id=2` 时，仅发送右臂参数，左臂自动补零
  - 当 `arm_id=0` 时，需要同时提供左右臂控制参数
